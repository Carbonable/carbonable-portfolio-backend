...
func (pq *ProjectQuery) Clone() *ProjectQuery {
	...
	return &ProjectQuery{
		...
		withTokens: pq.withTokens.Clone(),
...
func (pq *ProjectQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Project, error) {
	...
	if query := pq.withTokens; query != nil {
...
func (pq *ProjectQuery) loadTokens(ctx context.Context, query *CustomerTokensQuery, nodes []*Project, init func(*Project), assign func(*Project, *CustomerTokens)) error {
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(project.TokensTable)
		s.Join(joinT).On(s.C(customertokens.FieldID), joinT.C(project.TokensPrimaryKey[1]))
		s.Where(sql.InValues(joinT.C(project.TokensPrimaryKey[0]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(project.TokensPrimaryKey[0]))
	...
	neighbors, err := withInterceptors[[]*CustomerTokens](ctx, query, qr, query.inters)
	if err != nil {
		return err
	}
