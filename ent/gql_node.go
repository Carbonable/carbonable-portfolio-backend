...
func (c *Client) noder(ctx context.Context, table string, id int) (Noder, error) {
	switch table {
	case customertokens.Table:
		query := c.CustomerTokens.Query().
			Where(customertokens.ID(id))
		if fc := graphql.GetFieldContext(ctx); fc != nil {
			if err := query.collectField(ctx, true, graphql.GetOperationContext(ctx), fc.Field, nil, customertokensImplementors...); err != nil {
				return nil, err
			}
		}
...
func (c *Client) noders(ctx context.Context, table string, ids []int) ([]Noder, error) {
	...
	case customertokens.Table:
		query := c.CustomerTokens.Query().
			Where(customertokens.IDIn(ids...))
		query, err := query.CollectFields(ctx, customertokensImplementors...)
		if err != nil {
			return nil, err
		}
